...
def fetch_data():
    ...
    query = "SELECT * FROM readings"  # Make sure the table name is correct
...
# Add a title
st.title("PowerSight - CSED 6th Floor")
...
if graph_type == "Energy Consumption":
    if st.sidebar.button("Show Energy Consumption"):
        ...
        # Filter the data based on user inputs
        filtered_data = data.loc[start_date:end_date, ["Wattage (KwH)"]]
        ...
        # Line chart
        fig, ax = plt.subplots(figsize=(12, 6))
        ax = sns.lineplot(data=filtered_data, x=filtered_data.index, y="Wattage (KwH)", label="Energy Consumption")
        ax.set_xlabel("Date")
        ax.set_ylabel("Wattage (KwH)")
        ax.set_title("Energy Consumption over Time")
        ax.legend()
        st.pyplot(fig)

        # Interactive bar chart
        fig = px.bar(filtered_data, x=filtered_data.index, y="Wattage (KwH)", title="Energy Consumption")
        st.plotly_chart(fig)
        ...
        # Maximum and minimum values
        st.subheader("Maximum and Minimum Values")
        max_value = filtered_data["Wattage (KwH)"].max()
        min_value = filtered_data["Wattage (KwH)"].min()
        st.write(f"Maximum Energy Consumption: {max_value}")
        st.write(f"Minimum Energy Consumption: {min_value}")

elif graph_type == "Voltage/Amp":
    parameter = st.sidebar.selectbox("Select Parameter", ["Ampere", "PF"])

    if st.sidebar.button("Show Voltage/Amp"):
        start_date, end_date = st.sidebar.date_input("Select a Date Range", value=[data.index.min(), data.index.max()])

        # Filter the data based on user inputs
        filtered_data = data.loc[start_date:end_date, [parameter]]

        # Display visualizations
        st.header(f"{parameter}")
        ...
        # Interactive bar chart
        fig = px.bar(filtered_data, x=filtered_data.index, y=parameter, title=f"{parameter}")
        st.plotly_chart(fig)
